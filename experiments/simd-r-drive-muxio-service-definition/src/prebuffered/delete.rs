...
impl RpcMethodPrebuffered for Delete {
    ...
    fn encode_request(write_request_params: DeleteRequestParams) -> Result<Vec<u8>, io::Error> {
        Ok(bitcode::encode(&write_request_params))
    }

    fn decode_request(bytes: &[u8]) -> Result<Self::Input, io::Error> {
        let req_params = bitcode::decode::<DeleteRequestParams>(bytes)
    ...
    fn decode_response(bytes: &[u8]) -> Result<Self::Output, io::Error> {
        let resp_params = bitcode::decode::<DeleteResponseParams>(bytes)
            .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
