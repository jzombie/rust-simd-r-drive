...
pub struct ExistsRequestParams {
    pub key: Vec<u8>,
}

#[derive(Encode, Decode, PartialEq, Debug)]
pub struct ExistsResponseParams {
    pub exists: bool,
}

pub struct Exists;

impl RpcMethodPrebuffered for Exists {
    const METHOD_ID: u64 = rpc_method_id!("exists");

    type Input = ExistsRequestParams;
    type Output = ExistsResponseParams;
