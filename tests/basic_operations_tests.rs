...
fn test_open_existing_storage() {
    ...
    let storage = DataStore::open_existing(&path).expect("Failed to open existing storage file");
...
#[test]
fn test_read_with_key_hash() {
    let (_dir, storage) = create_temp_storage();
    let key = b"test_key_for_hash_read";
    let payload = b"some data";
    storage.write(key, payload).expect("Write failed");

    // Case 1: Read an existing key using its pre-computed hash
    let key_hash = compute_hash(key);
    let result = storage
        .read_with_key_hash(key_hash)
        .expect("Read with hash failed");

    assert!(result.is_some(), "Expected to find an entry with the hash");
    assert_eq!(result.unwrap().as_slice(), payload);

    // Case 2: Attempt to read a key that doesn't exist
    let missing_hash = compute_hash(b"a_key_that_was_never_written");
    let result_missing = storage
        .read_with_key_hash(missing_hash)
        .expect("Read with missing hash failed");

    assert!(
        result_missing.is_none(),
        "Expected to get None for a missing hash"
    );
}

#[test]
fn test_exists_with_key_hash() {
    let (_dir, storage) = create_temp_storage();
    let key = b"test_key_for_hash_exists";
    storage.write(key, b"some data").expect("Write failed");

    let existing_hash = compute_hash(key);
    let missing_hash = compute_hash(b"a_missing_key");

    // Case 1: Check for a key that exists
    assert!(
        storage.exists_with_key_hash(existing_hash).unwrap(),
        "Expected exists_with_key_hash to be true for an existing key"
    );

    // Case 2: Check for a key that does not exist
    assert!(
        !storage.exists_with_key_hash(missing_hash).unwrap(),
        "Expected exists_with_key_hash to be false for a missing key"
    );
}

#[test]
fn test_exists_with_key_hash_after_deletion() {
    let (_dir, storage) = create_temp_storage();
    let key = b"key_to_be_deleted";
    storage.write(key, b"data").expect("Write failed");

    let key_hash = compute_hash(key);

    // Verify it exists before deletion
    assert!(storage.exists_with_key_hash(key_hash).unwrap());
