...
use simd_r_drive::{
    DataStore,
    traits::{DataStoreReader, DataStoreStageWriter},
};
...
#[test]
fn stage_write_requires_explicit_flush() {
    let (_dir, store) = create_temp_storage(); // keep _dir alive!

    let key = b"key-a";
    let payload = b"value-a";

    let auto = store.stage_write(key, payload).expect("stage_write");
    ...
    store.stage_write_flush().expect("flush");

    let got = store.read(key).expect("post-flush read");
    assert_eq!(got.as_slice(), payload);
}

/// `stage_write()` must eventually return `true` once the buffer crosses its
/// soft-limit, but **no entry is persisted until we call `stage_write_flush()`**.
#[test]
fn stage_write_needs_flush_flag() {
    ...
    for i in 0usize..1_000_000 {
        ...
        let flag = store.stage_write(&key, &PAYLOAD).expect("stage_write");
    ...
    store.stage_write_flush().expect("explicit flush");
...
fn stage_write_double_flush_idempotent() {
    ...
    store.stage_write_flush().expect("first flush");
    ...
    store.stage_write_flush().expect("second flush");
